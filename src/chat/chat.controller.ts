...
export class ChatController {
  ...
  @MessagePattern({ cmd: 'mark_messages_delivered' })
  async markDelivered(@Payload() payload: MarkDeliveredDto) {
    const delivered = await this.chatService.markAsDelivered(
      payload.recipientId,
      payload.senderId,
    );
    const updatedIds = delivered.map((d) => d.id);
    return { ok: true, updatedCount: updatedIds.length, updatedIds };
  }
}